
<?php include 'head.php';?>
<section class="home_banner">
  <div class="home_banner_left">
    <div class="home_banner_left_content">
      <h2>National Leaders in Online<br>
        MBA test prep for</h2>
        <ul>
          <li><a href="#">CAT<span>|</span></a></li>
          <li><a href="#">XAT<span>|</span></a></li>
          <li><a href="#">IIFT<span>|</span></a></li>
          <li><a href="#">NMAT<span>|</span></a></li>
          <li><a href="#">SNAP<span>|</span></a></li>
          <li><a href="#">MHCET<span>|</span></a></li>
          <li><a href="#">CMAT<span>|</span></a></li>
        </ul>
    </div>
  </div>
  <div class="home_banner_right">
    <div class="home_banner_right_content">
      <h4>Announcements</h4>
      <ul>
        <li>Goa Institute of Management deadline: 15 Dec</li>
        <li>ITM Free Mini XAT | Take now</li>
        <li>Fund you own PGDM! Register for iFirst Scholarship</li>
        <li>SOIL PGPM Scholarships open now. Apply early</li>
        <li>Flash Sale - Flat 50% OFF on all courses</li>
        <li>SNAP Test Series @ Flat 50% OFF - Buy now</li>
      </ul>
    </div>
  </div>
</section><!--home banner-->
<section class="main_body">
  <div class="custom_container">
    <div class="main_body_left">
      <div class="study_pack">
        <div class="common_heading">
          <h3>Create Your Personalized <span>Study Pack</span></h3>
          <h2 class="background">
            <img class="bg" src="svg/home_common.svg">
          </h2>
        </div><!--common_heading-->
        <div class="exams">
          <h4>Exams:</h4>
          <ul>
          <li><a href="#">CAT<span>|</span></a></li>
          <li><a href="#">XAT<span>|</span></a></li>
          <li><a href="#">IIFT<span>|</span></a></li>
          <li><a href="#">NMAT<span>|</span></a></li>
          <li><a href="#">SNAP<span>|</span></a></li>
          <li><a href="#">MHCET<span>|</span></a></li>
          <li><a href="#">CMAT<span>|</span></a></li>
        </ul>
        <p>Select only the Test packs that YOU need</p>
        <button type="button" name="button" class="start_button">Create Your Pack Now</button>
      </div><!--exams-->
      <div class="pack_features">
        <h3>Pack <span>Features</span></h3>
        <div class="pack_slider">
          <ul class="bxflcarousel-studypack">
                <li>
                    <div class="grid-item">
                      <h4>UNLIMITED ASK-A-DOUBT</h4>
                      <p>Sample our demo FLTs simulating
                          actual mocks in the exam interface
                          and try our demo lessons </p>
                    </div>
                </li>
                <li>
                    <div class="grid-item">
                      <h4>DETAILED MOCK ANALYSIS</h4>
                      <p>Sample our demo FLTs simulating
                          actual mocks in the exam interface
                          and try our demo lessons </p>
                    </div>
                </li>
                <li>
                    <div class="grid-item">
                      <h4>ANALYTICS AND REPORTS</h4>
                      <p>Sample our demo FLTs simulating
                          actual mocks in the exam interface
                          and try our demo lessons </p>
                    </div>
                </li>
                <li>
                    <div class="grid-item">
                      <h4>UNLIMITED ASK-A-DOUBT</h4>
                      <p>Sample our demo FLTs simulating
                          actual mocks in the exam interface
                          and try our demo lessons </p>
                    </div>
                </li>
            </ul>
        </div>
      </div><!--pack features-->
      </div><!--study pack-->
      <div class="free_resources">
        <div class="common_heading">
          <h3>Free <span>Resources</span></h3>
          <h2 class="background">
            <img  class="bg" src="svg/home_common.svg">
          </h2>
        </div>
        <div class="resource_list">
          <ul class="clearfix">
  						<li>
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                	 viewBox="0 0 496.485 496.485" style="enable-background:new 0 0 496.485 496.485;" xml:space="preserve">
                <circle style="fill:#cd1b1b;" cx="213.333" cy="183.855" r="183.855"/>
                <rect x="72.921" y="229.624" style="fill:#FFFFFF;" width="387.879" height="260.655"/>
                <path style="fill:#000;" d="M460.8,496.485H72.921c-3.103,0-6.206-3.103-6.206-6.206V229.624c0-3.103,3.103-6.206,6.206-6.206
                	H460.8c3.103,0,6.206,3.103,6.206,6.206v260.655C467.006,493.382,463.903,496.485,460.8,496.485z M79.127,484.073h375.467V235.83
                	H79.127V484.073z"/>
                <path style="fill:#e1e6e9;" d="M242.812,212.558H106.279V440.63h136.533c13.188,0,24.048,10.085,24.048,21.721V234.279
                	C266.861,221.867,256,212.558,242.812,212.558z"/>
                <path style="fill:#000;" d="M266.861,468.558c-3.103,0-6.206-3.103-6.206-6.206c0-8.533-7.758-15.515-17.842-15.515H106.279
                	c-3.103,0-6.206-3.103-6.206-6.206V212.558c0-3.103,3.103-6.206,6.206-6.206h136.533c16.291,0,30.255,12.412,30.255,27.927v228.073
                	C273.067,465.455,269.964,468.558,266.861,468.558z M112.485,434.424h130.327c6.206,0,12.412,2.327,17.842,5.43V234.279
                	c0-8.533-7.758-15.515-17.842-15.515H112.485V434.424z"/>
                <rect x="132.655" y="238.933" style="fill:#FFFFFF;" width="107.055" height="174.545"/>
                <path style="fill:#e1e6e9;" d="M427.442,212.558H290.909c-13.188,0-24.048,10.085-24.048,21.721v228.073
                	c0-11.636,10.861-21.721,24.048-21.721h136.533V212.558z"/>
                <path style="fill:#000;" d="M266.861,468.558c-3.103,0-6.206-3.103-6.206-6.206V234.279c0-15.515,13.188-27.927,30.255-27.927
                	h136.533c3.103,0,6.206,3.103,6.206,6.206V440.63c0,3.103-3.103,6.206-6.206,6.206H290.909c-10.085,0-17.842,6.982-17.842,15.515
                	C273.067,465.455,269.964,468.558,266.861,468.558z M290.909,218.764c-10.085,0-17.842,6.982-17.842,15.515v205.576
                	c4.655-3.103,10.861-5.43,17.842-5.43h130.327V218.764H290.909z"/>
                <rect x="293.236" y="238.933" style="fill:#FFFFFF;" width="107.055" height="174.545"/>
                <g>
                	<rect x="346.764" y="300.994" style="fill:#000;" width="38.012" height="12.412"/>
                	<rect x="308.752" y="339.006" style="fill:#000;" width="76.8" height="12.412"/>
                	<rect x="308.752" y="377.018" style="fill:#000;" width="16.291" height="12.412"/>
                	<rect x="339.006" y="377.018" style="fill:#000;" width="16.291" height="12.412"/>
                	<rect x="369.261" y="377.018" style="fill:#000;" width="16.291" height="12.412"/>
                	<rect x="148.17" y="300.994" style="fill:#000;" width="76.8" height="12.412"/>
                	<rect x="148.17" y="339.006" style="fill:#000;" width="76.8" height="12.412"/>
                	<rect x="148.17" y="377.018" style="fill:#000;" width="16.291" height="12.412"/>
                	<rect x="178.424" y="377.018" style="fill:#000;" width="16.291" height="12.412"/>
                </g>
                <polygon style="fill:#E1E6E9;" points="72.921,102.4 72.921,257.552 245.139,257.552 317.285,313.406 317.285,257.552
                	354.521,257.552 354.521,102.4 "/>
                <path style="fill:#000;" d="M313.406,318.836l-70.594-54.303H72.145c-3.103,0-6.206-3.103-6.206-6.206V102.4
                	c0-3.103,3.103-6.206,6.206-6.206h281.6c3.103,0,6.206,3.103,6.206,6.206v155.927c0,3.103-3.103,6.206-6.206,6.206h-31.03v49.648
                	c0,2.327-1.552,4.655-3.879,5.43C318.061,320.388,315.733,320.388,313.406,318.836z M79.127,251.345h166.788
                	c1.552,0,3.103,0.776,3.879,1.552l62.061,48.097v-43.442c0-3.103,3.103-6.206,6.206-6.206h31.03V108.606H79.127V251.345z"/>
                <polygon style="fill:#FFFFFF;" points="253.673,232.727 97.745,232.727 97.745,127.224 329.697,127.224 329.697,232.727
                	292.461,232.727 292.461,262.206 "/>
                <g>
                	<path style="fill:#000;" d="M123.345,193.164l-4.655,13.188h-8.533l20.945-57.406h8.533l20.945,57.406h-8.533l-4.655-13.188
                		H123.345z M144.291,183.855l-8.533-23.273l-8.533,23.273H144.291z"/>
                	<path style="fill:#000;" d="M190.836,206.352h-20.945v-57.406h18.618c3.103,0,6.206,0.776,8.533,1.552
                		c2.327,0.776,3.879,2.327,5.43,3.879c2.327,3.103,3.103,6.206,3.103,10.085c0,4.655-1.552,7.758-3.879,10.085
                		c-0.776,0.776-1.552,1.552-1.552,1.552s-0.776,0.776-1.552,0.776c3.103,0.776,5.43,2.327,6.982,4.655
                		c1.552,2.327,2.327,5.43,2.327,9.309c0,3.879-0.776,7.758-3.103,10.861C201.697,204.024,197.042,206.352,190.836,206.352z
                		 M178.424,172.994h10.085c5.43,0,8.533-2.327,8.533-6.982c0-3.103-0.776-4.655-2.327-6.206c-1.552-1.552-3.879-1.552-6.206-1.552
                		h-10.085V172.994z M178.424,197.042h12.412c3.103,0,5.43-0.776,6.982-1.552s2.327-3.103,2.327-6.206
                		c0-5.43-3.103-7.758-10.085-7.758h-11.636V197.042z"/>
                	<path style="fill:#000;" d="M242.812,197.042c3.103,0,5.43-0.776,6.982-1.552c1.552-0.776,3.879-3.103,6.206-5.43l5.43,6.206
                		c-4.655,6.982-10.861,10.085-17.842,10.085s-13.188-3.103-17.842-8.533c-4.655-5.43-6.982-12.412-6.982-20.945
                		s2.327-15.515,6.982-20.945c4.655-5.43,10.861-8.533,17.842-8.533c6.982,0,13.188,3.103,18.618,10.085l-5.43,6.982
                		c-2.327-2.327-4.655-4.655-6.206-5.43c-2.327-0.776-4.655-1.552-6.982-1.552c-4.655,0-8.533,1.552-11.636,5.43
                		c-3.103,3.879-4.655,8.533-4.655,13.964c0,5.43,1.552,10.861,4.655,14.739C234.279,195.491,238.158,197.042,242.812,197.042z"/>
                	<path style="fill:#000;" d="M309.527,156.703c4.655,4.655,6.982,11.636,6.982,20.945c0,8.533-2.327,16.291-6.982,20.945
                		c-4.655,5.43-10.861,7.758-20.17,7.758h-15.515v-57.406h16.291C298.667,148.945,305.648,151.273,309.527,156.703z M308.752,177.648
                		c0-13.188-6.206-19.394-18.618-19.394h-7.758v39.564h8.533c5.43,0,10.085-1.552,13.188-4.655
                		C307.2,188.509,308.752,183.855,308.752,177.648z"/>
                </g>

                </svg>

  							<h4>DEMO <br>COURSEWARE</h4>
  							<p>Sample our demo FLTs simulating
                  actual mocks in the exam interface
                  and try our demo lessons </p>
  						</li>
  						<li>

                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                	 viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                <g>
                	<path style="fill:#000;" d="M423.724,26.483h17.655c9.751,0,17.655,7.904,17.655,17.655v326.621
                		c0,9.751-7.904,17.655-17.655,17.655H17.655C7.905,388.414,0,380.509,0,370.759V44.138c0-9.751,7.904-17.655,17.655-17.655H423.724
                		z"/>
                	<path style="fill:#b7b7b7;" d="M229.517,26.483v320c0.335-15.467-11.9-28.294-27.366-28.69H35.31V17.655
                		C35.339,7.916,43.227,0.028,52.966,0h150.069c7.027-0.009,13.768,2.778,18.736,7.746S229.527,19.456,229.517,26.483z"/>
                	<path style="fill:#7e7e7e;" d="M229.517,346.483v6.621H35.31v-35.31h166.841C217.618,318.189,229.852,331.015,229.517,346.483z"/>
                	<polygon style="fill:#cd1b1b;" points="141.241,150.069 105.931,105.931 70.621,150.069 70.621,0 141.241,0 	"/>
                	<path style="fill:#ececec;" d="M229.517,26.483v320c-0.335-15.467,11.9-28.294,27.366-28.69h166.841V17.655
                		C423.696,7.916,415.808,0.028,406.069,0H256c-7.027-0.009-13.768,2.778-18.736,7.746C232.295,12.715,229.508,19.456,229.517,26.483
                		z"/>
                	<path style="fill:#dbdbdb;" d="M229.517,346.483v6.621h194.207v-35.31H256.883C241.417,318.189,229.182,331.015,229.517,346.483z"
                		/>
                	<path style="fill:#ECF0F1;" d="M441.379,203.035h-52.966c-18.731-0.005-36.697,7.434-49.942,20.679s-20.684,31.211-20.679,49.942
                		v167.724c-0.005,18.731,7.434,36.697,20.679,49.942c13.245,13.245,31.211,20.684,49.942,20.679h52.966
                		c18.731,0.005,36.697-7.434,49.942-20.679c13.245-13.245,20.684-31.211,20.679-49.942V273.655
                		c0.005-18.731-7.433-36.697-20.679-49.942S460.111,203.029,441.379,203.035z"/>
                	<path style="fill:#BDC3C7;" d="M512,273.655v61.793h-97.103V203.035h26.483c18.731-0.005,36.697,7.434,49.942,20.679
                		S512.005,254.924,512,273.655z"/>
                	<path style="fill:#95A5A5;" d="M512,335.448v105.931c0.005,18.731-7.434,36.697-20.679,49.942
                		c-13.245,13.245-31.211,20.684-49.942,20.679h-52.966c-18.731,0.005-36.697-7.434-49.942-20.679
                		c-13.245-13.245-20.684-31.211-20.679-49.942V335.448H512z"/>
                </g>

                </svg>

  							<h4>DAILY <br>LEARNING</h4>
  							<p>Learn every day in three enriching
                  ways! - MBA Question of the day |
                  Test of the Day | Wordlist</p>
  						</li>
  						<li>
              
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                	 viewBox="0 0 512.001 512.001" style="enable-background:new 0 0 512.001 512.001;" xml:space="preserve">
                <circle style="fill:#cd1b1b;" cx="143.453" cy="368.548" r="143.453"/>
                <path style="fill:#a21515;" d="M214.303,243.792C226.2,264.693,233,288.872,233,314.642c0,79.227-64.226,143.453-143.453,143.453
                	c-25.77,0-49.949-6.8-70.851-18.697c2.603,4.574,5.464,8.98,8.538,13.221l-26.32,48.607c-3.444,6.36,3.498,13.303,9.859,9.859
                	l48.598-26.316C82.998,501.894,112.041,512,143.453,512c79.227,0,143.453-64.226,143.453-143.453
                	C286.905,315.09,257.663,268.469,214.303,243.792z"/>
                <path style="fill:#000;" d="M194.813,420.205l-5.457-5.457c6.322-10.219,9.983-22.251,9.983-35.125
                	c0-36.923-30.039-66.962-66.962-66.962S65.416,342.7,65.416,379.623s30.039,66.962,66.962,66.962c12.874,0,24.907-3.66,35.125-9.984
                	l5.457,5.457c3.017,3.017,6.972,4.527,10.926,4.527c3.954,0,7.91-1.508,10.926-4.527
                	C200.849,436.023,200.849,426.239,194.813,420.205z M132.378,415.678c-19.881,0-36.056-16.175-36.056-36.056
                	s16.175-36.056,36.056-36.056s36.056,16.175,36.056,36.056c0,4.245-0.743,8.32-2.097,12.106l-2.429-2.429
                	c-6.034-6.035-15.818-6.035-21.853,0c-6.035,6.035-6.035,15.818,0,21.853l2.429,2.429
                	C140.698,414.936,136.624,415.678,132.378,415.678z"/>
                <circle style="fill:#e1e6e9;" cx="332.491" cy="179.509" r="179.509"/>
                <path style="fill:#cbcfd2;" d="M477.929,284.731C499.357,255.165,512,218.816,512,179.509c0-63.364-32.833-119.057-82.411-151.003
                	c18.037,27.992,28.506,61.322,28.506,97.098c0,99.141-80.369,179.509-179.509,179.509c-35.776,0-69.105-10.47-97.098-28.506
                	c31.946,49.578,87.639,82.411,151.003,82.411c39.307,0,75.655-12.643,105.222-34.071l60.806,32.927
                	c7.959,4.309,16.647-4.378,12.337-12.337L477.929,284.731z"/>
                <path style="fill:#000;" d="M414.694,250.574L352.883,85.745c-2.261-6.032-8.027-10.027-14.469-10.027
                	c-6.442,0-12.208,3.995-14.469,10.027l-61.811,164.829c-2.997,7.99,1.052,16.898,9.043,19.895c7.99,2.997,16.897-1.052,19.895-9.043
                	l3.001-8.001h88.683l3.001,8.001c2.326,6.204,8.215,10.032,14.471,10.031c1.803,0,3.637-0.318,5.424-0.988
                	C413.641,267.472,417.691,258.565,414.694,250.574z M305.663,222.519l32.752-87.339l32.753,87.339H305.663z"/>

                </svg>

  							<h4>QUESTION <br>BANK</h4>
  							<p>Practise 10000+ Questions structured
                  through area-wise and difficulty-wise
                  tests</p>
  						</li>
              <li>


                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                	 viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                <polygon style="fill:#C2B8B9;" points="512,399.929 512,490.951 0,490.951 0,399.929 256,365.796 "/>
                <rect y="21.049" style="fill:#a6a6a6;" width="512" height="378.88"/>
                <circle style="fill:#cd1b1b;" cx="255.42" cy="210.489" r="124.188"/>
                <polygon style="fill:#000;" points="214.363,139.225 337.796,210.489 214.363,281.752 "/>
                <path style="fill:#fff;" d="M466.489,453.973H45.511c-4.713,0-8.533-3.82-8.533-8.533c0-4.714,3.821-8.533,8.533-8.533h420.978
                	c4.714,0,8.533,3.82,8.533,8.533C475.022,450.154,471.203,453.973,466.489,453.973z"/>

                </svg>


  							<h4>STRATEGY <br>VIDEOS</h4>
  							<p>Enhance your learning through XXX
                  hours of strategy and learning
                  videos </p>
  						</li>
              <li>
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                	 viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                <path style="fill:#e1e6e9;" d="M444.97,112V39.261c0-16.16-13.101-29.261-29.261-29.261H96.231C80.071,10,66.97,23.101,66.97,39.261
                	v433.478c0,16.16,13.101,29.261,29.261,29.261h319.478c16.16,0,29.261-13.101,29.261-29.261V333L444.97,112L444.97,112z"/>
                <polygon style="fill:#FFFFFF;" points="404.97,74 398.47,74 390.97,74 106.97,74 106.97,427.917 404.97,427.917 "/>
                <polygon style="fill:#fff;" points="312.468,198.68 312.468,251.021 250.911,278.104 189.353,251.021 189.353,198.68 "/>
                <path style="fill:#cd1b1b;" d="M352.47,176.307l-101.56,54.432l-101.56-54.432l101.56-54.432L352.47,176.307z"/>
                <polygon style="fill:#EDF2FA;" points="370.97,218 370.97,386 501.97,386 501.97,265.582 454.639,218 "/>
                <polygon style="fill:#cd1b1b;" points="454.639,219.84 454.639,265.582 500.14,265.582 "/>
                <polygon style="fill:#fff;" points="51.102,356.525 193.659,318.327 155.461,460.884 "/>
                <rect x="33.22" y="394.7" transform="matrix(-0.7071 -0.7071 0.7071 -0.7071 -194.3136 804.5419)" style="fill:#fff;" width="72.499" height="95.629"/>
                <path d="M66.97,214c2.63,0,5.21-1.07,7.069-2.931c1.86-1.859,2.931-4.439,2.931-7.069s-1.07-5.21-2.931-7.07
                	C72.18,195.07,69.6,194,66.97,194s-5.21,1.069-7.07,2.93s-2.93,4.44-2.93,7.07s1.069,5.21,2.93,7.069
                	C61.76,212.93,64.34,214,66.97,214z"/>
                <path d="M260.91,176.31c0-2.63-1.069-5.21-2.93-7.069c-1.86-1.87-4.44-2.931-7.07-2.931s-5.21,1.061-7.069,2.931
                	c-1.86,1.859-2.931,4.439-2.931,7.069s1.07,5.21,2.931,7.07c1.859,1.86,4.439,2.93,7.069,2.93s5.21-1.069,7.07-2.93
                	S260.91,178.939,260.91,176.31z"/>
                <path d="M352.47,214c5.522,0,10-4.478,10-10v-27.693c0-3.814-2.137-7.128-5.278-8.814l-101.558-54.431
                	c-2.951-1.582-6.496-1.582-9.447,0l-101.56,54.432c-3.249,1.74-5.276,5.128-5.276,8.813s2.027,7.073,5.276,8.813l34.726,18.611
                	v47.29c0,3.966,2.343,7.557,5.973,9.153l61.558,27.082c1.283,0.564,2.655,0.847,4.027,0.847s2.744-0.282,4.027-0.847l61.558-27.082
                	c3.63-1.597,5.973-5.188,5.973-9.153v-47.29l20.002-10.72V204C342.47,209.522,346.948,214,352.47,214z M250.911,133.221
                	l61.732,33.086h-25.237c-5.522,0-10,4.478-10,10s4.478,10,10,10h25.237l-61.732,33.086l-80.391-43.086L250.911,133.221z
                	 M302.468,244.496l-51.558,22.683l-51.558-22.683v-30.045l46.834,25.101c1.476,0.791,3.1,1.187,4.724,1.187s3.248-0.396,4.724-1.187
                	l46.834-25.101V244.496z"/>
                <path d="M509.06,258.529l-47.332-47.582c-1.798-1.808-4.218-2.846-6.758-2.931V39.261C454.97,17.612,437.358,0,415.71,0H96.231
                	C74.583,0,56.97,17.612,56.97,39.261V165c0,5.522,4.478,10,10,10s10-4.478,10-10V39.261C76.97,28.641,85.611,20,96.231,20H415.71
                	c10.62,0,19.261,8.641,19.261,19.261V208h-20V74c0-5.522-4.478-10-10-10h-298c-5.522,0-10,4.478-10,10v227.285
                	c0,5.522,4.478,10,10,10s10-4.478,10-10V84h278v124h-24c-5.522,0-10,4.478-10,10v168c0,5.522,4.478,10,10,10h24v21.917H177.326
                	L188.29,377h83.68c5.522,0,10-4.478,10-10s-4.478-10-10-10h-78.321l9.669-36.085c0.925-3.45-0.062-7.133-2.588-9.659
                	c-2.526-2.524-6.206-3.514-9.659-2.588L76.97,339.241V242.71c0-5.522-4.478-10-10-10s-10,4.478-10,10V344.6l-8.456,2.266
                	c-3.451,0.925-6.146,3.62-7.071,7.071c-0.925,3.45,0.062,7.133,2.588,9.659l19.477,19.477L2.957,443.625
                	c-3.905,3.905-3.905,10.237,0,14.143l51.262,51.262c1.953,1.953,4.512,2.929,7.071,2.929s5.118-0.977,7.071-2.929l60.552-60.551
                	l19.477,19.477c1.9,1.899,4.454,2.929,7.072,2.929c0.862,0,1.731-0.111,2.587-0.341c3.451-0.925,6.146-3.62,7.071-7.071
                	l6.848-25.555h233.003c5.522,0,10-4.478,10-10V396h20v76.739c0,10.62-8.641,19.261-19.261,19.261H123.957c-5.522,0-10,4.478-10,10
                	s4.478,10,10,10H415.71c21.648,0,39.261-17.612,39.261-39.261V396h47c5.522,0,10-4.478,10-10V265.582
                	C511.97,262.939,510.924,260.403,509.06,258.529z M61.29,487.816l-37.12-37.12l53.48-53.48l37.12,37.12L61.29,487.816z
                	 M150.284,441.565l-14.291-14.291c-0.003-0.003-0.006-0.007-0.009-0.01s-0.007-0.006-0.01-0.009l-51.243-51.243
                	c-0.003-0.003-0.006-0.007-0.009-0.01s-0.007-0.006-0.01-0.009l-14.291-14.291l109.096-29.232L150.284,441.565z M464.638,242.232
                	l13.279,13.35h-13.279V242.232z M380.97,376V228h63.668v37.582c0,5.522,4.478,10,10,10h37.332V376H380.97z"/>
                <path d="M318.282,309H235.97c-5.522,0-10,4.478-10,10s4.478,10,10,10h82.312c5.522,0,10-4.478,10-10S323.804,309,318.282,309z"/>
                <path d="M318.28,357c-2.63,0-5.21,1.069-7.069,2.93c-1.86,1.86-2.931,4.44-2.931,7.07s1.07,5.21,2.931,7.069
                	c1.859,1.86,4.439,2.931,7.069,2.931c2.641,0,5.21-1.07,7.07-2.931c1.86-1.859,2.93-4.439,2.93-7.069s-1.069-5.21-2.93-7.07
                	S320.921,357,318.28,357z"/>

                </svg>

  							<h4>ONLINE PAST <br>TESTS</h4>
  							<p>Check your preparedness on actual
                  exam tests in their actual
                  interface </p>
  						</li>
				  	</ul>
            <button type="button" name="button" class="start_button">Start Your Prep</button>
        </div><!--resourse list-->
      </div><!--free resources-->
      <div class="testimonials">
        <div class="common_heading">
          <h3>Testimonials</h3>
          <h2 class="background">
            <img src="svg/home_common.svg">
          </h2>
        </div>
        <div class="testimonial_slider">
          <ul class="bxflcarousel_testimonial">
                <li>
                    <div class="grid-item">
											<img src="images/testimonial-1.jpg">
    											<div class="slider-details">
    												<h4>Dr. Prabakaran John</h4>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <p>
                              <strong>Many desktop publishing packages </strong>and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will
                              uncover many web sites still in their infancy. <strong>Various versions </strong>have evolved over the years, sometimes by accident, sometimes on purpose.
                            </p>
    										  </div>
                    </div>
                </li>
                <li>
                    <div class="grid-item">
											<img src="images/testimonial-1.jpg">
  											<div class="slider-details">
  										    <h4>Dr. Prabakaran John</h4>
                          <span class="fa fa-star checked"></span>
                          <span class="fa fa-star checked"></span>
                          <span class="fa fa-star checked"></span>
                          <span class="fa fa-star"></span>
                          <span class="fa fa-star"></span>
                          <p>
                          <strong>Many desktop publishing packages </strong>and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will
                            uncover many web sites still in their infancy. <strong>Various versions </strong>have evolved over the years, sometimes by accident, sometimes on purpose.
                          </p>
  									    </div>
                    </div>
                </li>
            </ul>
        </div>
      </div><!--testimonials-->
    </div><!--main body left-->
    <div class="main_body_right">
      <ul>
        <li><img src="images/add-1.jpg"></li>
        <li><img src="images/add-2.jpg"></li>
        <li><img src="images/add-3.jpg"></li>
        <li><img src="images/add-4.jpg"></li>
        <li><img src="images/add-5.jpg"></li>
        <li><img src="images/add-6.jpg"></li>
        <li><img src="images/add-7.jpg"></li>
        <li><img src="images/add-8.jpg"></li>
        <li><img src="images/add-1.jpg"></li>
        <li><img src="images/add-2.jpg"></li>
        <li><img src="images/add-3.jpg"></li>
        <li><img src="images/add-4.jpg"></li>
      </ul>
    </div><!--main body right-->
  </div><!--custom container-->
</section><!--main body-->
<section class="counter_section">
  <div class="custom_container">
    <div class="row">
      <div class="col-md-4">
      <img src="svg/happy_students.svg">
        <div class=" stats-grid">
          <div class="stat-info">
             <h5>Happy Students</h5>
          </div>
          <div class="counter">8,02,828</div>
        </div>
      </div>
      <div class="col-md-4">
        <img src="svg/question_attempted.svg" alt="">
        <div class=" stats-grid">
          <div class="stat-info">
             <h5>Questions Attempted</h5>
          </div>
          <div class="counter">9,99,64,485</div>
        </div>
      </div>
      <div class="col-md-4">
       <img src="svg/test_taken.svg">
        <div class=" stats-grid">
          <div class="stat-info">
             <h5>Tests Taken</h5>
          </div>
          <div class="counter">2,75,416</div>
        </div>
      </div>
    </div>
  </div><!--custom container-->
</section><!--counter section-->
<?php include 'footer.php';?>
